...
public static class WhisperingEarringVakuuTakeTheWheelPatch
{
    ...
    private static async Task ModifiedBeforePlayPhaseStart(
        WhisperingEarring relic, PlayerChoiceContext choiceContext, Player player)
    {
        var combatState = (ICombatState)player.Creature.CombatState;
    
        using (CardSelectCmd.PushSelector(new VakuuCardSelector()))
        {
            int cardsPlayed = 0;
            int startTurn = relic.Owner.PlayerCombatState.TurnNumber;
        
            while (cardsPlayed < 13
                   && !CombatManager.Instance.IsOverOrEnding
                   && !CombatManager.Instance.IsPlayerReadyToEndTurn(player)
                   && relic.Owner.PlayerCombatState.TurnNumber == startTurn)
            {
                var card = PileType.Hand.GetPile(relic.Owner).Cards
    ...
    private static Creature GetTarget(WhisperingEarring relic, CardModel card, ICombatState combatState)
    {
        ...
        return card.TargetType switch
        {
            ...
            TargetType.AnyAlly => combatTargets.NextItem(
                combatState.Allies.Where(c => c != null && c.IsAlive && c.IsPlayer && c != relic.Owner.Creature)
            ),
            _ => null
        };
    }
